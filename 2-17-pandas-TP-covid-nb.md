...
#la connexion est bonne et on voit bien que ça affiche True et <class 'dict'> donc c'est bon
```
...
pays = 'France'
df = pd.DataFrame(by_country[pays])
df
```
...
```{code-cell} ipython3
# votre code
# 4 colonnes :date, confirmed, deaths, recovered
```
...
# votre code
pays2 = 'Italy'
df2 = pd.DataFrame(by_country[pays2])
df2['pays'] = pays2
df2
L=[]
L.append(df)
...
for pays, données in by_country.items():
    df = pd.DataFrame(données)
    df['pays'] = pays
    L.append(df)

global_l = pd.concat(L)
global_l
```
...
# votre code
#global_l.dtypes
nouvelle_date = pd.to_datetime(df['date'])
nouvelle_date

global_l['date'] = nouvelle_date
```
...
```{code-cell} ipython3
# votre code
final.loc['France', ['deaths', 'confirmed']].plot();
```
...
```{code-cell} ipython3
# votre code
df3 = final.loc[['France', 'Italy', 'Germany'], ['deaths', 'confirmed']]
```
...
def extract(pays, données,a: np.datetime64=None,
            b: np.datetime64=None):
    t = slice(a, b)
    return final.loc[(pays, t), données].unstack(level=0)
```
...
extract(['Zimbabwe', 'Germany'], ['deaths', 'confirmed']).plot();
```
...
#Il n'y a aucune mort pour le zimbabwe contrairement a l'allemagne, cependant les données deaths pour l'allemagne sont a 0 alors que les confirmed auglmentent (courbe rouge) donc ce n'est pas très logique.
```
