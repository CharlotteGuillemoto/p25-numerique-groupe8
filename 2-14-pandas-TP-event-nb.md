...
#pas réussi, le code reste rouge a chaque fois
```
...
```{code-cell} ipython3
# your code
df = events.set_index("beg").sort_index()

s = df["duration"].resample("W").sum()

plt.figure()
s.dt.total_seconds().div(3600).plot(kind="bar")
plt.ylabel("H")
plt.title('visu par semaine')
plt.show()


m = df["duration"].resample("ME").sum()

plt.figure()
m.dt.total_seconds().div(3600).plot(kind="bar")
plt.ylabel("H")
plt.title('visu par mois')
plt.show()

y = df["duration"].resample("YE").sum()

plt.figure()
y.dt.total_seconds().div(3600).plot(kind="bar")
plt.ylabel("H")
plt.title('visu par an')
plt.show()
```
...
he = s.map(convert_timedelta_to_hours)

plt.bar(he.index, he.values)

plt.ylabel("Durée(heures)")
plt.title("en semaines(heures)")
...
events = events.merge(countries[['country','region']], how='left', on='country')
events 
